...
const UpdateActivity = ({ activity, onClose }) => {
  ...
  const handleUploadImage = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error("Upload image failed", { autoClose: 1000 });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await postUpdateActivity(activity.id,{
        ...form,
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 h-screen">
      <div className="bg-white p-6 rounded-lg w-full max-w-3xl relative max-h-[85vh] overflow-y-auto custom-scroll">
        <button
          onClick={onClose}
          className="absolute top-2 right-3 text-3xl text-red-700 hover:text-red-500 cursor-pointer"
        >
          &times;
        </button>
